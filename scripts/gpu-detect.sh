...
install_mesa_stack() {
  ...
  if ! pkg_installed mesa && ! pkg_installed mesa-git; then
    packages+=("$MESA_PKG")
  fi

  if ! pkg_installed lib32-mesa && ! pkg_installed lib32-mesa-git; then
    packages+=("$LIB32_MESA_PKG")
  fi

  if [ "${#packages[@]}" -gt 0 ]; then
    sudo pacman -S --noconfirm --needed "${packages[@]}"
  else
    log_info "Mesa já está instalada (mesa ou mesa-git)"
  fi
}
